...
<script>
    ...
    fetch(`/api/product?search=${id}`, {
      ...
      .then(result => {
        ...
        result.data.forEach(value => {
          ...
          detailId.innerText = value._id
          detailAmount.innerText = value.total
          detailDate.innerText = value.time
          detailCashier.innerText = value.user
          console.log(detailId);
          console.log(detailAmount);
          console.log(detailDate);
          console.log(detailCashier);
  ...
  //         <button type="button" class="py-2 px-3 flex items-center text-sm font-medium text-center focus:outline-none rounded-lg border focus:z-10 focus:ring-4 focus:ring-gray-700 bg-gray-800 text-blue-400 border-blue-600 hover:text-white hover:bg-blue-700" onclick="actionButtonPress(this, 'prev')" id="prev-${content._id}">
...
<div id="readProductModal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full flex items-center justify-center" style="backdrop-filter: blur(10px); height:100%;">
  <div class="relative p-4 w-full max-w-xl max-h-full bg-gray-800 rounded-lg">
    ...
    <div class="flex justify-between items-center pb-4 mb-4 rounded-t border-b sm:mb-5 border-gray-600">
      <h3 class="text-lg font-semibold text-white" id="title-modal">Detail Invoice</h3>
    ...
    <div class="p-4 sm:p-4 overflow-y-auto">
      ...
      <div class="mt-5 sm:mt-10 grid grid-cols-2 sm:grid-cols-3 gap-5">
        <div>
          ...
          <span class="block text-sm font-medium text-gray-200" id="detail-amount"></span>
        </div>
        <div>
          <span class="block text-xs uppercase text-gray-500">Date paid:</span>
          <span class="block text-sm font-medium text-gray-200" id="detail-date"></span>
        </div>
        <div>
          <span class="block text-xs uppercase text-gray-500">Cashier</span>
          <div class="flex items-center gap-x-2">
            <span class="block text-sm font-medium text-gray-200" id="detail-cashier"></span>
          </div>
        </div>
      </div>
